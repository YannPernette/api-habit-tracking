    ...
    (err, globalHabits) => {
      if (err)
        return res.status(500).json({
          message: "Erreur lors de la récupération des habitudes globales",
        });
    ...
    db.get(
      ...
      async (err, user) => {
        ...
        if (user)
          return res
            .status(400)
            .send({ message: "Nom d'utilisateur déjà pris" });
  ...
  db.run(
    ...
    function (err) {
      if (err)
        return res
          .status(500)
          .json({ message: "Erreur lors de la création de l'habitude" });
  ...
  db.run(
    ...
    function (err) {
      if (err)
        return res
          .status(500)
          .json({ message: "Erreur lors de l'enregistrement du suivi" });
